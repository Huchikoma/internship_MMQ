...
def get_schema_id( xsd_fname='am_upload.xsd' ):
    user_name = "root"
...
def process( scenario=1, xsd_fname='am_schema_R2018a.xsd', xsd_dirname='schema', xml_dirname='xmlFiles'):
    ...
    if ( scenario==1 or scenario==2 ):
        ...
        list_docs=[]#
        mylist = listdir(xml_data_dirpath)#
        mylist.sort()#

        print '- status: uploading xml files from xml path [' + xml_dirname + ']'

        for xml_filename in mylist:#listdir(xml_data_dirpath):
            with open(join(xml_data_dirpath, xml_filename), "r") as xml_file:
                ...
                if str(data["title"])==str(xml_filename) and str(xml_filename) not in list_docs:#
